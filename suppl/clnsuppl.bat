...
for %%i in (compile.me suppl*.lib suppl*.lst vars.bat) do if exist %%i del %%i>nul
for %%i in (echolib.bat echoto.bat *.cfg) do if exist %%i del %%i>nul
IF NOT EXIST src\NUL goto noSrc

cd src
FOR %%A in (*.obj) DO DEL %%A
for %%i in (suppl*.lib suppl*.lst) do if exist %%i del %%i>nul
for %%a in (supplib.rsp echolib.bat echoto.bat *.cfg) do if exist %%a del %%a>nul
